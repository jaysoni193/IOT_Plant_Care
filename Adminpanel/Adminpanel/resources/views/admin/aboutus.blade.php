        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
              ...
              <div class="mb-3">
                ...
                <input type="text" name="userid" class="form-control" id="recipient-name">
              </div>
              <div class="mb-3">
                <label for="message-text" class="col-form-label">Tempearture:</label>
              ...
              <div class="card-body">
                <div class="table-responsive">
                  <table class="table">
                    ...
                    <tbody>
                      @foreach($aboutus as $date)
                      <tr>
                      <td> {{$date->id}} </td>
                      <td> {{$date->device_id}} </td>
                      <td> {{$date->user_id}} </td>
                        <td> {{$date->Temperature}} </td>
                      <td> {{$date->	Mositure}} </td>  
                        <td> {{$date->Humidity}} </td> 
                        <td>
                        <a href="{{url('about-us/'.$date->id)}}" class="btn btn-success">EDIT</a>
                        </td>
                        <td>
                        <form method="POST" action="{{ route('abouts.delete', $date->id) }}">
                          <input type="hidden" name="_token" value="{{ csrf_token() }}">
                          {{method_field('DELETE')}}
